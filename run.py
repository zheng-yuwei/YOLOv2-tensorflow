...
def train(yolov2_detector):
    ...
    yolov2_detector.train(train_dataset, None)
    logging.info('训练完毕！')


def test(yolov2_detector, yolov2_decoder, save_path=None):
    """
    YOLO v2模型测试
    :param yolov2_detector: yolov2检测模型
    ...
    for batch_counter, (images, labels, image_paths) in enumerate(test_set):
        ...
        predictions = yolov2_detector.predict(images)
...
def predict(yolov2_detector, yolov2_decoder, image_paths, save_path):
    """
    YOLO v2模型预测
    :param yolov2_detector: yolov2检测模型
    :param yolov2_decoder: yolov2模型输出解码器
    ...
    for image_path in image_paths:
        ...
        predictions = yolov2_detector.predict(np.expand_dims(image, axis=0))
        predictions, predict_boxes = yolov2_decoder.decode(predictions)
...
def run():
    ...
    yolov2_detector = YOLOV2Trainer()
    
    # 模型训练
    if FLAGS.mode == 'train':
        train(yolov2_detector)
    # 多GPU模型，需要先转为单GPU模型，然后再执行测试
    elif FLAGS.mode == 'test' or FLAGS.mode == 'predict':
        # 多GPU模型转换为单GPU模型
        if FLAGS.gpu_num > 1:
            yolov2_detector.convert_multi2single()
            logging.info('多GPU训练模型转换单GPU运行模型成功，请使用单GPU测试！')
            return
        # 进行测试或预测
        yolov2_decoder = YOLOV2Decoder(grid_size=FLAGS.output_grid_size, class_num=FLAGS.class_num,
        ...
        if FLAGS.mode == 'test':
            test(yolov2_detector, yolov2_decoder, save_path)
            logging.info('测试结束！！！')
        else:
            ...
            predict(yolov2_detector, yolov2_decoder, image_paths, save_path)
            logging.info('预测结果！！！')
    # 将模型保存为pb模型
    elif FLAGS.mode == 'save_pb':
        # 保存模型记得注释eager execution
        yolov2_detector.save_mobile()
    # 将模型保存为服务器pb模型
    elif FLAGS.mode == 'save_serving':
        # 保存模型记得注释eager execution
        yolov2_detector.save_serving()
    else:
