...
class YOLOv2PostProcessor(object):
    """ YOLO v2 后处理函数 """
    ...
    def apply_nms(boxes, nms_thresh):
        ...
        while index < box_num:
            ...
            boxes_iou = [(i, YOLOv2PostProcessor._cal_iou(sorted_boxes[index], box)) for (i, box) in same_class_boxes]
    ...
    def _cal_iou(box, truth):
        ...
        :return 两个box的iou
        """
        w = YOLOv2PostProcessor._overlap(box[0], box[2], truth[0], truth[2])
        h = YOLOv2PostProcessor._overlap(box[1], box[3], truth[1], truth[3])
        if w <= 0 or h <= 0:
            return 0
        inter_area = w * h
    ...
    def visualize(image, boxes, src_box_size, image_path):
        ...
        for box in boxes:
            ...
            cv2.rectangle(image, (int(round(left)), int(round(top))), (int(round(right)), int(round(bottom))),
                          (255, 242, 35), max(1, round(3 * image_width / 1200)))
